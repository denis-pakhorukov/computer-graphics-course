...
void OnInit() {
    ...
    shader.AddUniform("current_time");
    shader.UnUse();
    ...
    for (j = 0; j <= NUM_Z; j++) {
        for (i = 0; i <= NUM_X; i++) {
            vertices[count++] = glm::vec3(((float(i) / (NUM_X - 1)) * 2 - 1) * HALF_SIZE_X, 0,
...
//display callback
void OnRender() {
    //get the elapse current_time
    current_time = glutGet(GLUT_ELAPSED_TIME) / 1000.0f * SPEED;
    ...
    glUniform1f(shader("current_time"), current_time);
